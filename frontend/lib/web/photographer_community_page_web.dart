...
class _PhotographerCommunityPageState
  ...
  Future<void> _openAddPost({bool asQuestion = false}) async {
    final created = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => AddCommunityPostPageWeb(
  ...
  Future<void> _openDetails(Map post) async {
    ...
    final changed = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => CommunityPostDetailsPageWeb(
          postId: postId,
        ),
      ),
    );
                        ...
                        if (crossAxisCount == 1) {
                          return SliverList(
                            delegate: SliverChildBuilderDelegate(
                              (context, index) {
                                final post =
                                    Map<String, dynamic>.from(posts[index]);
                  ...
                  GestureDetector(
                    onTap: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (_) => const CommunityReelsPageWeb(),
                        ),
                      );
                    },
                    child: Container(
