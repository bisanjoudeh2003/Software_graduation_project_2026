...
class _ClientBookingDetailsPageState
    extends State<ClientBookingDetailsPage> {
  ...
  String prettyDate(String? d) {
    if (d == null) return "";
    try { return DateFormat("EEEE, MMM d yyyy").format(DateTime.parse(d)); }
    catch (_) { return d; }
  }
  ...
  // هل الأونر هو اللي لغى؟
  // لو status = cancelled وكان deposit_paid = 1 → الأونر لغى ورجع العربون
  ...
  void _showMsg(String msg, {bool success = false}) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(20)),
        title: Text(success ? "✓ Done" : "Notice",
            style: TextStyle(fontFamily: "Montserrat",
                fontWeight: FontWeight.bold,
                    ...
                    children: [
                      ...
                      _card([
                        ...
                        _row(Icons.access_time_rounded, "Time",
                            "$start → $end"),
                        _divider(),
                        _row(Icons.tag_rounded, "Booking ID",
                            "#${booking['id']}"),
                      ]),
                      ...
                      const SizedBox(height: 10),
                      _card([
                        _row(Icons.receipt_long_rounded, "Total Price",
                            "\$${totalPrice.toStringAsFixed(0)}"),
                        _divider(),
                        _row(Icons.payments_rounded, "Deposit (30%)",
